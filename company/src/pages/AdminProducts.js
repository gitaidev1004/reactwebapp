...
useEffect(() => {
  getProducts(page)
    .then(data => {
      setProducts(data.results); // 상품 리스트
      setTotalPages(Math.ceil(data.count / 10)); // 총 페이지 계산
    })
    .catch(err => {
            ...
            <div style={{ width: '100%', maxWidth: 1448, margin: '0 auto', padding: '20px', paddingTop:100 }}>
                ...
                <div style={{ marginTop: 30, display: 'flex' }}>
                    ...
                    <div style={{ width: '100%', padding: '10px 0', borderLeft: '1px solid #e5e5e5' }}>
                        ...
                        <Link to="/products/add" style={{ textAlign: 'center', fontSize: 13, margin: 20, maxWidth: 60, display: "block", backgroundColor: '#000', padding: 5, color: '#fff' }}>상품등록</Link>
                        <div style={{ padding: 20 }}>
                            <table className="table table-striped">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th style={{ paddingLeft: 20 }}>이미지</th>
                                        <th>상품명</th>
                                        <th>가격</th>
                                        <th></th>
                                    </tr>
                                ...
                                <tbody>
                                    {products.map(product => (
                                        <tr key={product.id}>
                                            ...
                                            <td style={{ width: '20%', padding: 20 }}><img
                                                src={`${product.image}`}
                                                alt={product.name}
                                                style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                                            /></td>
                                            <td style={{ width: '40%', verticalAlign: 'middle' }}><a style={{ color: '#000', fontWeight: 'bold' }} href={`/products/${product.id}`}>{product.name}</a></td>
                                            <td style={{ width: '20%', verticalAlign: 'middle' }}>{Number(product.price).toLocaleString()}원</td>
